	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeGitHubClient := fakerestclient.CreateHTTPClient(func(request *http.Request) (*http.Response, error) {
				...
				// Check mode
				if gotMode := request.FormValue("hub.mode"); gotMode != string(tt.params.mode) {
					t.Errorf("doGitHubHubbubRequest() expected hub.mode %s; got: %s", string(tt.params.mode), gotMode)
				}
				// Check callback

...
var (
	...
	// webhookEvents are the events webhooks are created on
	webhookEvents = []string{"push", "pull_request"}
)
...
// createWebhook creates a webhook for repositories that do not exist on the
// EventListener and then creates/updates the EventListener with new Triggers
func (cg *Group) createWebhook(request *restful.Request, response *restful.Response) {
	...
	el, err := cg.getWebhookEventListener()
	...
	// Update or create EventListener Kubernetes resource
	if eventListenerExists {
		if err := cg.updateEventListener(el); err != nil {
			util.RespondError(response, err, http.StatusInternalServerError)
			return
		}
	} else {
		if err := cg.createEventListener(el); err != nil {
			util.RespondError(response, err, http.StatusInternalServerError)
			return
		}
		// Await EventListenerStatus to be populated
		el, err = cg.waitForEventListenerStatus()
		if err != nil {
			util.RespondError(response, err, http.StatusInternalServerError)
			return
		}
		...
		if strings.Contains(strings.ToLower(cg.Defaults.Platform), "openshift") {
			if err := cg.createOpenshiftRoute(el.Status.Configuration.GeneratedResourceName); err != nil {
				...
				util.RespondError(response, xerrors.New("Failed to create Route for webhook"), http.StatusInternalServerError)
				return
			}
		} else {
			if err := cg.createIngress(el.Status.Configuration.GeneratedResourceName); err != nil {
				logging.Log.Debug("Failed to create Ingress, deleting EventListener...")
				if err = cg.deleteEventListener(); err != nil {
					logging.Log.Debug("Failed to delete EventListener")
				}
				util.RespondError(response, xerrors.New("Failed to create Ingress for webhook"), http.StatusInternalServerError)
				return
			}
		}
	}
...
// deleteWebhook attempts to remove a webhook and then the corresponding
// triggers on the EventListener
func (cg *Group) deleteWebhook(request *restful.Request, response *restful.Response) {
	...
	if err := model.ValidateWebhookName(name); err != nil {
		util.RespondError(response, err, http.StatusBadRequest)
		return
	}
	...
	if deleteWebhookPipelineRuns {
		if err := cg.deletePipelineRuns(gitURL, deleteWebhook.Namespace, deleteWebhook.Pipeline); err != nil {
			util.RespondError(response, err, http.StatusInternalServerError)
			return
		}
	}
...
func (cg *Group) deletePipelineRuns(repoURL *url.URL, namespace, pipeline string) error {
...
func (cg *Group) createOpenshiftRoute(serviceName string) error {
...
func (cg *Group) deleteOpenshiftRoute(routeName string) error {
...
func (cg *Group) createIngress(serviceName string) error {
...
func (cg *Group) deleteIngress(ingressName string) error {
...
// to be persisted
func addWebhookTriggers(cg *Group, eventListener *triggersv1alpha1.EventListener, webhook model.Webhook) {
...
// persisted
func removeWebhookTriggers(eventListener *triggersv1alpha1.EventListener, webhookName string) {
	newTriggers := []triggersv1alpha1.EventListenerTrigger{}
...
func getMonitorTriggerParams(cg *Group, w model.Webhook) []pipelinesv1alpha1.Param {
...
func getPipelineTriggerParams(w model.Webhook) []pipelinesv1alpha1.Param {
...
func triggerToWebhook(t triggersv1alpha1.EventListenerTrigger) (*model.Webhook, error) {
...
func filterWebhooksByRepo(webhooks []model.Webhook, repoURL string) []model.Webhook {
	filteredWebhooks := []model.Webhook{}
...
func findWebhookByName(webhooks []model.Webhook, name string) (*model.Webhook, error) {
	for _, webhook := range webhooks {
...
func (cg *Group) getWebhookEventListener() (*triggersv1alpha1.EventListener, error) {
...
func (cg *Group) createEventListener(el *triggersv1alpha1.EventListener) error {
...
func (cg *Group) updateEventListener(el *triggersv1alpha1.EventListener) error {
...
func (cg *Group) deleteEventListener() error {
...
// created.
func (cg *Group) waitForEventListenerStatus() (*triggersv1alpha1.EventListener, error) {
	for {
...
func (cg *Group) getDashboardURL() string {
	type element struct {

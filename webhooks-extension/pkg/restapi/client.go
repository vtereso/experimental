...
const (
	...
	platform = "PLATFORM"
	// sslEnabled is the ENV that specifies whether the webhook should use SSL
	sslEnabled = "SSL_VERIFICATION_ENABLED"
)
...
type EnvDefaults struct {
	...
	// Platform is the platform the Dashboard extension is deployed on. On
	// Openshift, routes are created for webhooks
	Platform string `json:"platform"`
	// SSLEnabled specifies whether webhooks use SSL
...
func NewGroup() (*Group, error) {
	...
	defaults := EnvDefaults{
		...
		SSLEnabled:  os.Getenv(sslEnabled),
	}
